...
<note>
Generate agents 40-50 lines only. Always route through meta-prompt-critic for validation. Use agent-generation.prompt.md template for consistency.
</note>
...
<formatting>
Generated specialist agent with sections:
- **Agent File:** Complete .agent.md ready for deployment
- **Validation Checklist:** Safety, logic, and compliance verification
- **Deployment Instructions:** Setup and lifecycle management steps
- **Performance Baseline:** Expected execution time and success criteria
</formatting>

<examples>
<example>
<input>Analyze Kubernetes YAML for security vulnerabilities</input>
<output>Novel domain (K8s security) requiring specialized agent. Generate k8s-security-auditor (40-50 lines) using agent-generation.prompt.md. Validate through meta-prompt-critic. Route to ecosystem-orchestrator for execution. Output: Security vulnerability report</output>
</example>
</examples>
